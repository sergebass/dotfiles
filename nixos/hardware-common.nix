...
{ config, lib, pkgs, modulesPath, ... }: let
  btrfsListFailedInodesScript = with pkgs; writeShellScriptBin "btrs-list-failed-inodes" ''
    dmesg | grep -Po 'csum failed root \d+ ino\S* \d+' | awk '{print $6}' | sort -nu
  '';

  btrfsListFailedFilesScript = with pkgs; writeShellScriptBin "btrs-list-failed-files" ''
